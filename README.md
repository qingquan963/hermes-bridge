...
│   ├── ResultWriter.cpp/h
│   ├── StateFile.cpp/h
│   ├── HttpServer.cpp/h       # 内置 HTTP 服务器（用于接收回调）
│   ├── CallbackWriter.cpp/h   # 回调文件写入
│   └── handlers\
│       ├── IHandler.cpp/h
...
| `tcp_connect` | 建立 TCP 代理连接（通过 Bridge） | `host`, `port` |
| `tcp_send` | 通过已建立的 TCP 连接发送数据 | `connection_id`, `data` |

> **TCP 代理说明**：Bridge 在 Windows 侧维护 TCP 连接池，通过 callback 机制异步接收数据。
